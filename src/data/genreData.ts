/**
 * 장르(Genres) 상세 소개 페이지용 대규모 정적 데이터
 * 
 * 타투의 주요 장르들을 상위 카테고리( Heritage, Contrast 등)로 묶고,
 * 각 하위 장르(Irezumi, Blackwork 등)에 대한 심층 역사, 특징, 이미지를 제공합니다.
 */

/**
 * 상위 메뉴 탭에 노출될 대분류 카테고리와, 
 * 해당 카테고리에 속하는 실제 타투 장르(Sub-genre)들의 매핑 테이블
 */
...
/**
 * 각 대분류 카테고리 탭을 클릭했을 때 상단에 표시되는 
 * 카테고리 전체를 관통하는 핵심 요약 설명 문구
 */
...
/**
 * 장르 상세 설명 내의 개별 문단이나 리스트를 구성하는 구획(Section) 타입
 */
export type ContentSection = {
    title: string;                                    // 구획의 소제목 (예: "1. 기원과 역사")
    items?: { subtitle?: string; text: string }[];    // 일반 서술형 항목 리스트
    list?: { term: string; desc: string }[];          // 용어 정의형 리스트 (예: 주요 도안과 의미)
    text?: string;                                    // 도입부 등 단일 문단 텍스트
};

/**
 * 개별 타투 장르의 전체 정보를 담는 스키마 구조
 */
export type GenreInfo = {
    desc: string;               // 장르의 핵심을 한 줄로 요약한 메인 설명
    features: string[];         // 눈에 띄게 표시될 3가지 주요 해시태그/특징 키워드
    details?: ContentSection[]; // 하단에 펼쳐질 상세 기원, 역사, 특징 섹션들
};

/**
 * 각 하위 장르 이름(예: "Irezumi")을 키로 하여
 * 실제 상세 데이터 모델(GenreInfo)을 반환하는 거대한 매핑 객체
 */
            ...
            },
            {
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        text: "검은색 점을 촘촘히 찍거나(도트워크), 타투 머신을 빠르게 튕겨내듯 그어(윕 쉐이딩) 거칠고 입자감이 느껴지는 질감으로 명암 표현을 합니다."
                    },
                    {
